...
除了全局的黑白名单配置，GoProxy 还支持在发起请求时，通过代理认证的 **密码 (Password)** 字段动态指定该次请求所需的出口国家。

### 工作机制

1. **认证校验**：系统仅校验 **用户名 (Username)** 是否匹配（必须与配置一致），忽略密码的正确性。
2. **过滤解析**：将请求中的 **密码 (Password)** 解析为国家代码列表（以分号 `;` 分隔，大小写不敏感）。
3. **动态筛选**：仅从符合这些国家代码的代理节点中随机/最优选择。

### 使用示例

| 场景 | 代理认证信息 | 说明 |
|------|------------|------|
| 不限国家 | `user:` | 密码留空，使用全局过滤规则 |
| 仅限美国 | `user:US` | 仅返回 US 节点 |
| 美日韩任选 | `user:US;JP;KR` | 在指定国家节点中轮换 |

**命令行 cURL 示例**：
```bash
# 动态请求美国节点
curl -x http://proxy:US@localhost:7777 https://httpbin.org/ip

# 动态请求日本或新加坡节点
curl -x http://proxy:JP;SG@localhost:7777 https://httpbin.org/ip
```

> **注意**：按需过滤是在全局过滤（`ALLOWED_COUNTRIES` / `BLOCKED_COUNTRIES`）的基础上进行二次筛选。即：你只能请求到当前池子中已有的、且符合全局安全策略的国家节点。
